...
Event = Shelf.Model.extend
  ...
  approve: (req, res, next) ->
    ...
    .then (host) =>
      ...
      .fetch()
      .then (host) =>
        host.sendEmail('meetup-approved', 'Your meetup has been approved!', {url: 'https://worlddominationsummit.com/meetup/'+this.get('slug')})
        @set('active', 1)
        @set('ignored', 0)
        @save()
        setTimeout ->
  ...
  hosts: ->
    ...
    .fetch({columns: ['first_name', 'last_name', 'users.user_id', 'user_name', 'host_type']})
    .then (rsp) ->
      hosts = rsp.models
      dfr.resolve(hosts)
    , (err) ->
