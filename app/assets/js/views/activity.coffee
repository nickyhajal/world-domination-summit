ap.Views.activity = XView.extend
	atnTimo: 0
	events:
	...
	initialize: ->
		...
		_.whenReady 'assets', =>
			@options.activity = ap.Events.getBySlug(@options.activity)
			@options.out = _.template @options.out, @options.activity.attributes
			@options.sidebar = 'activity'
			@options.sidebar_filler = @options.activity.attributes
			@initRender()
	...
	renderMap: ->
		_.whenReady 'googlemaps', =>
			attendee = @options.activity.attributes
	...
	renderHosts: ->
		html = ''
		for host in @options.activity.attributes.hosts
	...
	renderAttendees: ->
		...
		ap.api 'get event/attendees', {event_id: @options.activity.get('event_id')}, (rsp) =>
