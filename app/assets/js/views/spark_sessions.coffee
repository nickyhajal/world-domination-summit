ap.Views.spark_sessions = XView.extend
	saveScrollPosition: true
	initialize: ->
		@options.sidebar = 'activities'
		@initRender()
	...
	renderEvents: ->
		...
		ap.Events.each (ev) =>
			...
			if ev.get('type') is 'spark_session' # and +time.format('X') > (now-30000)
				maxed = false
				...
				event_html = '
					<div class="meetup-descr-shell'+maxed_class+'">
						<div class="meetup-content">
							...
							<div class="meetup-descr-who">A Spark Session for '+ev.get('who')+'</div>
